...
| 6 | **`Failed to poll currentLevel` `genLevelCtrl.read(currentLevel)` on EP4 + EP5 → `Status 'UNSUPPORTED_ATTRIBUTE'`** | 27+27 (2026-09-05 08:42…) + 25+25 zh-level (2026-09-04 23:04…) — the ONLY recurring error-level noise | **Z2M core `dist/extension/bind.js:505-553` — `POLL_ON_MESSAGE` mechanism (source-verified)**: after traffic on bound clusters, core debounces ~1 s and reads poll attributes (`currentLevel` for genLevelCtrl) from endpoints passing a `supportsInputCluster` gate — EP4/5 pass (genLevelCtrl IS advertised as input), firmware answers `UNSUPPORTED_ATTRIBUTE`, core logs error at `bind.js:547`. Armed by the *restored, documented pre-rejoin* `genLevelCtrl→coordinator` binds. | YES — `onOff` on the same EPs read/poll fine seconds before & after; LED/indicator attrs intact | **EXPECTED/BIND-ARTIFACT — not BSEED converter (0× `currentLevel` refs in all three converter files), not V7 (bindings identical pre-rejoin under V6-era config; rejoin simply stopped the poll; restore restarted it). Firmware quirk: EP4/5 ADVERTISE genLevelCtrl in input cluster list but do NOT implement `currentLevel` → descriptor/attribute mismatch, V6-invariant.** |
