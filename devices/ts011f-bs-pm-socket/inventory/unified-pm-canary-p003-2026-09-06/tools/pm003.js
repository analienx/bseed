'use strict';
// P003 finalization runner (issue #11 APPROVED / OTA-CANARY ruling 5560609637). One-shot predecessor-OTA attempt
...
function verifyArtifact() {
    ...
    fs.writeFileSync(OUT + '/hashes.txt', ['file_name: ' + REQ.file_name, 'bytes: ' + data.length, 'sha256: ' + sha, 'source_repo: analienx/tuya-zigbee-switch PR#5', 'source_SHA: ' + REQ.sourceSHA + ' (reconfirmed 2026-09-06 pre-P003)', 'actions_run: ' + REQ.sourceRun + ' artifact: ' + REQ.artifact, 'header: ' + JSON.stringify(hdr)].join('\n') + '\n');
...
function connect(cb) {
    ...
    const m = cfg.mqtt;
    const client = mqtt.connect(m.server, {clientId: 'pm003-' + MODE + '-' + Math.random().toString(16).slice(2, 6), username: m.user, password: m.password, reconnectPeriod: 0, connectTimeout: 10_000, keepalive: 60});
...
if (MODE === 'preflight') {
    ...
    connect(async (client, cfg) => {
        ...
        const preflight = {
            timestamp: now(), proposal: 'P003', authorization: 'issue#11 comment 5560609637 APPROVED / OTA-CANARY',
    ...
    connect(async (client) => {
        ...
        client.on('message', (topic, payload) => {
            try {
                const s = payload.toString();
                if (!/^zigbee2mqtt\/(bridge\/(logging|response)|LivingRoomSocketWifiLeft)/.test(topic)) return;
                ...
                if (topic === 'zigbee2mqtt/bridge/logging') {
                    if (/Updating 'LivingRoomSocketWifiLeft'/i.test(msg) && !imageOffered) { imageOffered = true; queryLike = 'INFO_Updating_seen'; }
                    ...
                    if (/OTA update of 'LivingRoomSocketWifiLeft' failed/i.test(msg)) { terminal = terminal || 'failed'; terminalAt = terminalAt || now(); }
        ...
        if (!powerCycled) {
            ...
            fs.writeFileSync(OUT + '/result.json', JSON.stringify({result: 'P003_POWER_CYCLE_UNAVAILABLE', unschedule: u, finishedAt: now()}, null, 1));
            console.log('RESULT P003_POWER_CYCLE_UNAVAILABLE'); client.end(true); process.exit(13);
        }
        const tr = Date.now();
...
} else { console.error('usage: pm003.js preflight|run|postcheck|canary'); process.exit(2); }
