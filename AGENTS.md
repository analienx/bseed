...
3. `docs/SAFETY.md`;
4. `policy/CLASS_A_CLOSURE.md`;
5. `policy/BRICK_THREAT_MODEL.md`;
6. `policy/OTA_REVERSIBILITY.md`;
7. `policy/EMPIRICAL_DEVELOPMENT.md`;
8. `PROJECT_STATUS.md`;
9. the assigned GitHub issue;
10. control issue #1 latest comments before any mutation approval or execution.
...
The Supervisor is the authoritative engineering agent. It owns architecture, firmware code/tests, upstream integration, Class A closure, brick-prevention and safety/recovery invariants, empirical experiment design, task decomposition, mutation approvals, evidence review and project status.
...
Class A facts are facts whose wrong value could cause unsafe GPIO drive, wrong-target firmware, hardware damage, or loss of OTA/recovery. They are not permitted to remain hypotheses during firmware experimentation.

- Issue #3 closes hardware Class A facts A-H01..A-H14.
- Issue #5 closes recovery Class A facts A-R01..A-R07.
- `scripts/class_a_gate.py --mode all` must PASS with `class_a_unknown_count=0` before any experimental OTA proposal can pass `preflash_gate.py`.

Calibration constants, pulse timing, edge preference, filtering, low-load thresholds and similar runtime behavior are not Class A; those are deliberately discovered later in the volatile diagnostic PM sandbox.
...
1. `scripts/class_a_gate.py --mode all` PASSes for the exact canary with zero unknowns;
...
9. `scripts/preflash_gate.py` PASSes the fresh live device state and matching Class A report;
...
Class A closure is stricter: hardware facts must be `DEVICE_CONFIRMED`; recovery facts must be `RECOVERY_PROVEN`.

Source provenance may additionally be recorded as `SOURCE_CONFIRMED`, but datasheet/upstream evidence never substitutes for observation on the exact target board where Class A requires device confirmation.
